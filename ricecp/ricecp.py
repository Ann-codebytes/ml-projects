...
print(y.shape)
train_x, test_candidate_x, train_y, test_candidate_y = train_test_split(image_list, y, test_size=0.2, random_state=0)

test_x,val_x,  test_y, val_y = train_test_split(test_candidate_x, test_candidate_y, test_size=0.1, random_state=0)
...
data_loader_rice_train = torch.utils.data.DataLoader(rice_data_train,batch_size=32,shuffle=True)

x=val_x
#x=StandardScaler().fit_transform(x)
y=val_y
rice_data_val = RiceDataset()
data_loader_rice_val = torch.utils.data.DataLoader(rice_data_val,batch_size=32,shuffle=True)

x=test_x
#x=StandardScaler().fit_transform(x)
y=test_y
rice_data_test = RiceDataset()
data_loader_rice_test = torch.utils.data.DataLoader(rice_data_test,batch_size=700,shuffle=True)
