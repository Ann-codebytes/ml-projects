...
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s',filename=os.path.join(os.getcwd(),'trainlog.log'), filemode='w')
...
def list_all_image_pairs(base_dir):
    ...
    for family in families:
        ...
        for relative in os.listdir(os.path.join(families_directory, family)):
            ...
            for pic in os.listdir(os.path.join(relative_dir, relative)):
                ...
                resize_img = np.asarray(Image.open(os.path.join(pic_dic, pic)).convert('L'))
                x.append(resize_img)
    ...
    for ind in range(0, len(related)): #len(related)
        try:
            z[int(related.iloc[ind]['p1'].split('/')[0][1:])][int(related.iloc[ind]['p1'].split('/')[1][3:])].append(
                int(related.iloc[ind]['p2'].split('/')[1][3:]))
            z[int(related.iloc[ind]['p1'].split('/')[0][1:])][int(related.iloc[ind]['p2'].split('/')[1][3:])].append(
...
def get_images_for_training(x, y, w,z, size_list=64):
    ...
    train_x.append(np.zeros((size_list, 224, 224, 1))) # final size is (2, batch_size, 224, 224, 1)
    train_x.append(np.zeros((size_list, 224, 224, 1)))
    ran_values = np.random.randint(0,2, (1,size_list)) # Random value which determines whether to chose from relatives or not , I need both categories for training
    rand_indexes_0 = np.random.choice(range(0, len(x)), size_list, replace=False)  # Random Indexes for choosing the first half of images
    for ind_batchy in range(0, size_list):  # Creating the 2nd half of pictures either from relatives or non-relatives
...
def print_vals(train_x,train_y):
    ...
    # print(len(fiwd_data))
    print(fiwd_data.__getitem__(0)) #this returns, image 1, image 2 and label that is expected
...
if __name__=="__main__":
    ...
    for epoch in range(num_epochs):
        for (img1,img2,y) in data_loader_db:
            optimizer.zero_grad()
            output1,output2=net(torch.tensor(img1.float()),torch.tensor(img2.float()))
