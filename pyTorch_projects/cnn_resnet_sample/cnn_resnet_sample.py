...
train_dataset = torchvision.datasets.CIFAR10(
    root="C://", train=True, transform=transform, download=False
)
test_data_set = torchvision.datasets.CIFAR10(
    root="C://", train=False, transform=transforms.ToTensor()
)
...
test_data_loader = torch.utils.data.DataLoader(
    dataset=test_data_set, batch_size=batch_size, shuffle=False
)
...
class ResNet(nn.Module):
    ...
    def make_layer(self, resBlock, out_channels, res_blocks, stride=1):
        ...
        residual_blocks.append(
            resBlock(self.in_channels, out_channels, stride, downsampling)
        )
