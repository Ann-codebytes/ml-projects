...
def denormalize_image(input):
    input=input.numpy().transpose((1,2,0)) #size of this input will be cxhxw ---> h,w,c
...
for epoch in range(500):
    ...
    content_loss_mse_val = torch.mean((content_features[-1] - generated_features[-1])**2)
    style_loss = 0
    ...
    optimizer.step()
    if epoch%10 == 0:
        print(f"Epoch: {epoch} Content loss: {content_loss_mse_val.item()} Style loss: {style_loss.item()} Total loss: {total_loss.item()}")
