...
class CNN(nn.Module):
    ...
    def forward(self,x):
        x = F.relu(self.conv1(x)) #output size  =[ (256-5 +(2 *0)) / stride] + 1, that is 256 is pixels, 5 is the kernel size, 2* padding of zero divided by stride that is 1 => 252x252
        x=self.pool(x) #max pool with 2 so we reduce size by 2, so we get 126x126
        x = F.relu(self.conv3(x))#output size in this case is [(126 - 5 + 2(0))/1]+1=122x22
...
checkpoint = {'epoch':1, 'model_state_dict':model.state_dict(), 'optimizer_state_dict':optimizer.state_dict(),'loss':0.2}
